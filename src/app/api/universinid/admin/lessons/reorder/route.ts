...
export async function POST(request: NextRequest) {
  ...
  if (!parsed.parentId) return apiError('parentId (moduleId) é obrigatório', 400);
  const parentId = parsed.parentId;
  const updated = await prisma.$transaction(
    parsed.items.map((it) => prisma.lesson.updateMany({ where: { id: it.id, moduleId: parentId }, data: { position: it.position } })),
  );
