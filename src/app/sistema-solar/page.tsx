...
/** Reads `?view=pipeline|universinid` and primes the store. */
function ApplyViewFromQuery() {
  const params = useSearchParams();
  const setView = useStore((s) => s.setView);
  useEffect(() => {
    const v = params.get('view');
    if (v === 'pipeline' || v === 'universinid') setView(v);
  }, [params, setView]);
  return null;
}
    ...
    <main className="relative h-screen w-screen overflow-hidden">
      ...
      <header className="pointer-events-none absolute top-0 left-0 right-0 px-10 py-6 z-10 flex items-start justify-between gap-6">
        <div className="flex items-center gap-4">
          <div
            className="h-12 w-12 bg-gradient-to-br from-delp-orange to-delp-yellow"
